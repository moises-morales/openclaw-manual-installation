...
1. [Docker Desktop](https://www.docker.com/products/docker-desktop/) corriendo.
2. [XQuartz](https://www.xquartz.org/) instalado — necesario para mostrar la ventana del juego.

---

## 1. Preparar display X11 (macOS)

```bash
# Abre XQuartz y activa "Allow connections from network clients"
...
```bash
cd /Users/moy5ix/code/agents-ai/openclaw-manual
...
## 4. Instalación manual de OpenClaw (dentro del contenedor)

### 4.1 Clonar el repositorio
```bash
cd /openclaw
git clone https://github.com/pjasicek/OpenClaw.git source
cd source
```
...
### 4.3 Copiar recursos del juego
OpenClaw necesita los assets originales del juego Commander Keen: Alien Menace (CLAW.REZ).
Copia el archivo `CLAW.REZ` a `/openclaw/assets/` en tu Mac; ya está montado en el contenedor.

```bash
# Dentro del contenedor:
ls /openclaw/assets/   # debe aparecer CLAW.REZ
```

### 4.4 Ejecutar OpenClaw
```bash
cd /openclaw/source/build
./openjazz                  # nombre puede variar; busca el binario
# o
./openclaw --rezfile /openclaw/assets/CLAW.REZ
```
...
| Acción | Comando |
|--------|---------|
| Ver logs del contenedor | `docker compose logs -f` |
| Parar el contenedor | `docker compose stop` |
| Eliminar contenedor + red | `docker compose down` |
...
- El volumen `openclaw-build` persiste `/openclaw` entre reinicios del contenedor. La compilación no se pierde.
- La carpeta local `./assets` se monta en `/openclaw/assets` — coloca ahí `CLAW.REZ`.
- Si el display no funciona, verifica que XQuartz esté abierto y `xhost + 127.0.0.1` haya sido ejecutado **después** de abrir XQuartz.
